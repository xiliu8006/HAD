...
class CameraPoseInterpolator:
    ...
    #     """
    #     if np.isnan(pose1).any() or np.isnan(pose2).any():
    #         print("Warning: Input poses contain NaN values")
    #         return np.nan
    ...
    def compute_pose_distance(self, pose1, pose2):
        ...
        if np.isnan(translation_dist):
            print("Warning: Translation distance is NaN")
            return np.nan
        ...
        try:
            ...
            # Check quaternions for NaN
            if np.isnan(q1).any() or np.isnan(q2).any():
                print("Warning: Quaternions contain NaN values")
                return np.nan
            ...
            # Check if dot product is valid for arccos
            squared_dot = 2 * np.dot(q1, q2) ** 2 - 1
            if squared_dot > 1 or squared_dot < -1:
                # print(f"Warning: Invalid value for arccos: {squared_dot}")
                # print("pose1 is and pose 2 is: ", pose1, pose2)
                return 0
                
            rotation_dist = np.arccos(squared_dot)
            
            if np.isnan(rotation_dist):
                print("Warning: Rotation distance is NaN")
                return np.nan
    ...
    def find_nearest_assignments(self, training_poses, testing_poses, top=None):
        """
        ...
        for j in range(M):
            ...
            if top is not None:
                # Find the top N nearest training poses
                top_indices = np.argsort(distances)[:top]
                assignments.append(top_indices)
    ...
    def shift_poses(self, training_poses, testing_poses, distance=0.1, threshold=0.1):
        ...
        for test_idx, train_idx in enumerate(assignments):
            train_pose = training_poses[train_idx]
            test_pose = testing_poses[test_idx]
            # print("test_idx is and distance is: ", test_idx, self.compute_pose_distance(train_pose, test_pose))
